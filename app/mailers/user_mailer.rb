...
class UserMailer < ActionMailer::Base
  ...
  def email_confirmation(user)
    ...
    mail(to: "#{@user.full_name} <#{@user.email}>", subject: I18n.t('models.sys_user.actions.confirm_email'), locale: I18n.locale)
  end
  ...
  def restore_password(user)
    ...
    mail(to: "#{@user.full_name} <#{@user.email}>", subject: I18n.t('models.sys_user.actions.restore'), locale: I18n.locale)
  end
end
